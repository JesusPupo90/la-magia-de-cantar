...
- **Implementado:**
  ...
  - **Email de confirmación de pago (Resend, §7):** `lib/email.ts` (`sendPaymentConfirmation` con plantilla de bienvenida + expectativas del equipo; FROM con nombre visible `La Magia de Cantar <no-reply@...>`), `lib/orders/send-confirmation.ts` (`maybeSendConfirmation` con guarda anti-duplicados `orders.confirmation_sent_at`), disparado desde `lib/orders/process-payment.ts` (approved) y el webhook. Scripts de prueba: `scripts/test-resend.ts`, `scripts/test-payment-email.ts`.
